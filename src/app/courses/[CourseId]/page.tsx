...
import { cardData, apps, Chapter, ChapterDescription } from '../../_data/data';

// Fetch course data from the mock data source
async function fetchCourseData(courseId: string) {
    // Simulate fetching data
    const course = apps.find((course) => course.id === courseId);
    const data = cardData.find((data) => data.id === courseId);
    return { course, data };
}
...
export default async function Details({ params }: DetailsProps) {
    ...
    const { course, data } = await fetchCourseData(CourseId);

    function getChapters(details: Chapter) {
        return (
            <div className='w-full'>
                <Accordion allowMultiple>
                    <AccordionItem className='bg-white border-1 border-black border-solid w-full'>
                        ...
                        <AccordionPanel pb={4}>
                            {
                                details.chapterDescription.map((description: ChapterDescription, index) => (
                                    <div key={index} className='lg:p-4 mb-4 mt-4 flex items-center justify-between w-full'>
                                        <h1 className='font-varela text-lg pl-8 flex items-center w-fit'>
            ...
            <div className='grid lg:grid-cols-10 grid-cols-1 items-center justify-stretch lg:p-10'>
                <div className='p-8 w-full lg:col-span-7'>
                    ...
                    <div className='p-6 bg-white rounded-xl'>
                        {course?.details.map((details, index) => (
                            <div key={index}>
                                {getChapters(details)}
                            </div>
                        ))}
